...
@Component({
  selector: 'app-search-tab-spares',
  templateUrl: './search-tab-spares.component.html',
  styleUrls: ['./search-tab-spares.component.scss']
})
export class SearchTabSparesComponent implements OnInit,OnChanges {
  @Input() searchContent!: string;
  @Output() sparesCount = new EventEmitter<number>();
    ...
    getSearchProduct(value:any){
     ...
     this.IsLoader =  true;
       if(value.length>2){
        ...
        this._products = res;
        this.sparesCount.emit(this._products.length);
        this.IsLoader =  false;
