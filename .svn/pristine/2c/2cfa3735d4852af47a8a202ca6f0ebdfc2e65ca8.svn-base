...
// import { VolvoBusEntity } from 'src/app/models/proposal/cqrs';
// import { ApiService } from 'src/app/services/api.service';

// @Component({
//   selector: 'app-volvo-bus',
//   templateUrl: './volvo-bus.component.html',
//   styleUrls: ['./volvo-bus.component.scss']
// })
// export class VolvoBusComponent {
//   @Output() arrayEmitter = new EventEmitter<string[]>();
//   @ViewChild('dropdown') dropdown: ElementRef | undefined;

//   @Input() cqrsID:number=0;
//   @Input() prospectCode:string="";
//   volvoBusData: VolvoBusEntity;
//   volvoBusTableData: VolvoBusEntity[] = [];
//   options: string[] = [];
//   selectedItems: any[] = [];
//   newItem: string = '';
...
//     this.volvoBusData = new VolvoBusEntity();
    
//   }
//   ngOnInit(): void {
//     this.getVolvoBusData();
//   }

//   onSubmit()
//   {
//     this.volvoBusData.cqrsId=this.cqrsID;
//     this.volvoBusData.prospectcode=this.prospectCode;
//     this.volvoBusData.roomNames=JSON.stringify(this.selectedItems);
//     this.volvoBusData.flag="INSERT";
//     console.log(this.volvoBusData)
//     this.ApiService.httpost(this.volvoBusData,'/Proposal/saveVolvoBus')
//     .subscribe((res:any)=>{
//       this.getVolvoBusData();
...
//   getVolvoBusData() {
//     this.ApiService.httpgetMaster("&ProspectCode="+this.prospectCode+"&cqrsId="+this.cqrsID+"", "/Proposal/getVolvoBusData").subscribe(
//       (response: any) => {
//         console.log(response.data);
//         this.volvoBusTableData=response.data;
//         this.sendArray();
//       },
//       (err) => {
//       }
...
// const roomNamesArray = [].concat(...this.volvoBusTableData.map(item => JSON.parse(item.roomNames))); 
// //this.volvoBusTableData.map(item => JSON.parse(item.roomNames));
...
count:number=0
counter(){
  this.count=this.EPData.NoOfEPRooms
this.count=this.count+1
// alert(this.count)
// alert(this.EPData.NoOfEPRooms)
}
...
tempData(e:string)
{
  ...
  if(!this.editOpen)
  {
    ...
    // this.toShow=false
      ...
      this.EPData.NoOfEPRooms=this.EPData.noOfEPRooms
      this.onSubmit()
      this.counter()
      if(this.EPData.EPRooms.length>=this.EPData.NoOfEPRooms||this.EPData.epRooms.length>=this.EPData.NoOfEPRooms)
      {
